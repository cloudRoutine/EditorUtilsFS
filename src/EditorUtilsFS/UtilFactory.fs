...
type UtilFactory =
    ...
    /// </summary>
    static member CreateTaggerRaw<'Data,'Tag when 'Tag :> ITag> (basicTaggerSource:IBasicTaggerSource<'Tag >) =
        new BasicTagger<'Tag> (basicTaggerSource)
    ...
    static member CreateTagger<'Tag when 'Tag :> ITag> 
            (   propertyCollection  :   PropertyCollection                      ,
                key                 :   obj                                     ,
                createFunc          :   unit -> IBasicTaggerSource<'Tag> ) : ITagger<'Tag> =
        let makeBasicTagger = 
            ( fun()-> new BasicTagger<'Tag>(createFunc()) :> ITagger<'Tag>)

        new CountedTagger<'Tag> (   propertyCollection  , 
                                    key                 , 
                                    makeBasicTagger     ) :> ITagger<'Tag>
    ...
    /// </summary>
    static member CreateOutlinerTagger( textBuffer:ITextBuffer ) : ITagger<IOutliningRegionTag> =
        let makeBasicTagger = (fun() -> UtilFactory.GetOrCreateOutlinerCore(textBuffer) :> IBasicTaggerSource<IOutliningRegionTag>)
